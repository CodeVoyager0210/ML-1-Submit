# -*- coding: utf-8 -*-
"""
Model Training Phase Implementation
阶段四：模型训练 - Model Training

This module implements the model training phase with cross-validation,
performance tracking, and comprehensive evaluation.
"""
...
class ModelTrainer:
    ...
    def __init__(self, data_dir: str = "split_data", models_dir: str = "models",
        ...
        self.learning_curves_dir = self.plots_dir / "learning_curves"
        self.learning_curves_dir.mkdir(parents=True, exist_ok=True)
        ...
        logger.info(f"ModelTrainer initialized with data_dir={data_dir}, models_dir={models_dir}")
    ...
    def train_single_model(self, model_name: str, model: Any, X_train: pd.DataFrame,
        ...
        try:
            ...
            # 生成学习曲线
            learning_curve_path = self.generate_learning_curves(
                model, model_name, fold, X_train, y_train
            )
    ...
    def generate_learning_curves(self, model: Any, model_name: str, fold: int,
                                X: pd.DataFrame, y: pd.Series) -> Optional[str]:
        """
        生成学习曲线
        ...
        try:
            ...
            plt.title(f'Learning Curves - {model_name} (Fold {fold})')
            plt.legend()
            plt.grid(True)

            # 保存图表
            learning_curve_path = self.learning_curves_dir / f"{model_name}_fold_{fold}_learning_curves.png"
            plt.savefig(learning_curve_path, dpi=300, bbox_inches='tight')
            plt.close()

            logger.info(f"Generated learning curves for {model_name} fold {fold}")

            return str(learning_curve_path)

        except Exception as e:
            logger.error(f"Error generating learning curves for {model_name} fold {fold}: {str(e)}")
            return None
    ...
    def _save_results(self, all_results: List[Dict], analysis: Dict[str, Any]):
        """保存结果到文件"""
        # 保存详细结果
        results_path = self.metrics_dir / "training_results.json"
        with open(results_path, 'w') as f:
            json.dump(all_results, f, indent=2, default=str)

        # 保存分析结果
        analysis_path = self.metrics_dir / "training_analysis.json"
        with open(analysis_path, 'w') as f:
            json.dump(analysis, f, indent=2, default=str)

        # 保存模型排名CSV
        ranking_df = pd.DataFrame(analysis['model_ranking'])
        ranking_path = self.reports_dir / "model_ranking.csv"
        ranking_df.to_csv(ranking_path, index=False)

        logger.info(f"Results saved to {results_path}")

    def _generate_visualization_report(self, all_results: List[Dict]):
        """生成可视化报告"""
        try:
            # 创建性能对比图
            ...
            r2_scores = [r['avg_val_metrics']['r2'] for r in all_results]
            ...
            # RMSE对比
            rmse_scores = [r['avg_val_metrics']['rmse'] for r in all_results]
            ...
            # 训练时间对比
            training_times = [r['total_training_time'] for r in all_results]
            axes[1, 0].bar(model_names, training_times, color='lightgreen')
            axes[1, 0].set_title('Training Time Comparison')
            axes[1, 0].set_ylabel('Time (seconds)')
            axes[1, 0].tick_params(axis='x', rotation=45)

            # 训练vs验证R²对比
            train_r2 = [r['avg_train_metrics']['r2'] for r in all_results]
            val_r2 = [r['avg_val_metrics']['r2'] for r in all_results]

            x = np.arange(len(model_names))
            width = 0.35

            axes[1, 1].bar(x - width/2, train_r2, width, label='Training R²', color='blue', alpha=0.7)
            axes[1, 1].bar(x + width/2, val_r2, width, label='Validation R²', color='red', alpha=0.7)
            ...
            # 保存图表
            comparison_path = self.plots_dir / "model_comparison.png"
            plt.savefig(comparison_path, dpi=300, bbox_inches='tight')
            plt.close()

            logger.info(f"Generated visualization report: {comparison_path}")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='Model Training Phase')
    ...
    parser.add_argument('--test-mode', action='store_true',
                       help='Run in test mode (single fold)')
    ...
    try:
        ...
        trainer = ModelTrainer(
            ...
            cv_folds=1 if args.test_mode else args.cv_folds
        )
